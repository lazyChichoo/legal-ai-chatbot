    ...
    for _mi, msg in enumerate(st.session_state.messages):
        with st.chat_message(msg["role"]):
            ...
            with st.expander("查看本次依据法条与重写记录"):
                ...
                if msg["provisions"]:
                    # 判卷同学反馈：这里应该亮出"来源法条的条号"，而不是直接把
                    # 知识库整段正文码上来。所以法律依据放大写在前面，知识库原文
                    # 收进下面的开关里（答辩时要证明"出处真实"再打开）。
                    for p in msg["provisions"]:
                        st.markdown("**法律依据：%s**" % p["source"])
                        st.caption("对应知识库第 %s 条 · %s ｜ 场景：%s"
                                   % (p["no"], p["title"], p["scene"]))
                    st.info("以上是本次 AI 仅可使用的参考材料，知识库其余内容本次不会被调用。")
                    if st.checkbox("显示知识库条文原文", key="raw_%d" % _mi):
                        for p in msg["provisions"]:
