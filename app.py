...
# CSS：输入框sticky贴底（不写死left，适配侧边栏拖拽）+ 合同报告字号控制
st.markdown("""
<style>
div.block-container {
    padding-bottom: 20px;
...
.contract-report-text p { font-size: 14px !important; }
.contract-report-text strong { font-size: 14px !important; }
</style>
...
# ========== 法律问答：重构消息渲染，新消息不掉框外，历史出处不丢失 ==========
with tab_chat:
    # 第一步：先处理用户新输入，append到messages（在渲染container之前，确保新消息在框内）
    ...
    # 第二步：统一在固定高度容器内渲染所有消息（包括刚append的新消息）
